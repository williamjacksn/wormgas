...
class RainwaveCog(discord.ext.commands.Cog, name="Rainwave"):
    ...
    async def key_add(
        ...
        await ctx.author.send(
            f"I assigned the key {rainwave_key} to {ctx.author.mention}."
        )
    ...
    def build_embed(song: dict) -> discord.Embed:
        ...
        embed = discord.Embed(
            title=song["title"],
            colour=discord.Colour(0xF7941E),
    ...
    def build_embed_ustats(user: dict) -> discord.Embed:
        ...
        embed = discord.Embed(
            title=user_name,
            url=user_url,
            description="Rainwave user stats",
            colour=user_colour,
        )
        ...
        embed.add_field(
            name="Game",
            value=f"{user.get('rating_completion').get('1')}% rated",
            inline=True,
        )
        embed.add_field(
            name="Chiptune",
            value=f"{user.get('rating_completion').get('4')}% rated",
            inline=True,
        )
        embed.add_field(
            name="OC ReMix",
            value=f"{user.get('rating_completion').get('2')}% rated",
            inline=True,
        )
        embed.add_field(
            name="Covers",
            value=f"{user.get('rating_completion').get('3')}% rated",
            inline=True,
        )
        return embed
    ...
    async def ustats(
        ...
        async with ctx.typing():
            ...
            if username is None:
                ...
                username.startswith("<@")
                and username.endswith(">")
                and username[2:-1].isdigit()
            ):
            ...
            if current_channel:
                embed.set_footer(
                    text=f"Currently listening to the {current_channel.long_name}"
                )
